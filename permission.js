...
function getNextRoute(to) {
  if (to.path.startsWith('/mobile')) {
    ...
    return {
      loginPath: '/login', rootPath: '/'
    }
  }
}

const whiteList = ['/login', '/mobile/login', '/authredirect']// no redirect whitelist
        ...
        store.dispatch('GetInfo').then(res => { // 拉取user_info
          ...
          store.dispatch('GenerateRoutes', { roles }).then(() => { // 根据roles权限生成可访问的路由表
            ...
            store.dispatch('GetSession').then(res => {
              store.dispatch('GetShopConfig', store.getters.sid)
            })
          ...
          store.dispatch('FedLogOut').then(() => {
            ...
            next(getNextRoute(to).loginPath)
            // next({ path: '/' })
          })
        })
      } else {
