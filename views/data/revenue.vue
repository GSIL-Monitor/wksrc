...
<template>
	<div class="app-container">
		...
		<el-table :key="tableKey" :data="list" v-loading="listLoding" element-loading-text="稍等片刻" border  style="width: 100%">
			...
			<el-table-column align="center" label="抹零金额">
				<template slot-scope="scope">
					<span>{{scope.row.inputdiscVal}}</span>
				</template>
			</el-table-column>
...
<script >
	//引用路由方法 发送请求
	import {revenueData,PrintRevenueList} from '@/api/data'
	...
	export default{
		...
		methods:{
			...
			row.sid = this.listQuery.sid
			PrintRevenueList(row).then(response => {
                this.$notify({
                    title: '打印营收数据',
                    message: response.msg,
                    type: response.code === 200?'success' : 'danger',
			...
			handlePrintSum(row){
				...
				if(this.listQuery.createDT != '' && this.listQuery.createDT !=undefined){
					tempData.dataDT = this.listQuery.createDT[0] +' <--> ' +this.listQuery.createDT[1];
				}
				tempData.wxPay = tempData.SwxPay
				tempData.cashPay = tempData.ScashPay
				tempData.cardPay = tempData.ScardPay
				tempData.entertain = tempData.Sentertain
				tempData.payTreasure = tempData.SpayTreasure
				tempData.xfjPay = tempData.SxfjPay
				tempData.lefePay = tempData.SlefePay
				tempData.tCardPay = tempData.StCardPay
				tempData.turnCard = tempData.SturnCard
				tempData.transfer = tempData.Stransfer
				tempData.oneCard = tempData.SoneCard
				tempData.webPay = tempData.SwebPay
				tempData.sumFund = tempData.SsumFund
				tempData.orderSum = tempData.SorderSum
				tempData.inputdiscVal = tempData.SinputdiscVal
				console.log(tempData)
              PrintRevenueList(tempData).then(response => {
                this.$notify({
                    title: '打印营收总计数据',
                    message: response.msg,
                    type: response.code === 200?'success' : 'danger',
