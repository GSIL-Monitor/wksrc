...
<div class="app-container">
  <div class="filter-container">
    ...
    <el-select clearable  style="width: 140px" v-model="listQuery.upid" class="filter-item" placeholder="所属区域">
				<el-option v-for="item in areaOptions" :key="item.id" :label="item.name+'('+item.id+')'" :value="item.id"></el-option>
		</el-select>
    <el-input @keyup.enter.native="handleFilter" style="width: 100px;" class="filter-item" placeholder="工作区域名称" v-model="listQuery.name">
    </el-input>
  ...
  <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
            style="width: 100%">
    <el-table-column align="center" label="工作区域ID" width="65">
      <template slot-scope="scope">
        <span>{{scope.row.id}}</span>
      </template>
    </el-table-column>
    <el-table-column align="center"  label="工作区域名称" width="120">
      <template slot-scope="scope">
    ...
    <el-table-column align="center"  label="所属区域" width="120">
      <template slot-scope="scope">
        <span>{{scope.row.upid | displayNameFilter(areaOptions)}}</span>
      </template>
    </el-table-column>
  ...
  <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
    <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="120px" style='margin-left:0px;'>
      <el-form-item label="店铺" prop="sid">
        <el-select v-model="temp.sid" placeholder="" @change="getArea()">
          <el-option v-for="(item,index) in shopOptions" :key="item.id" :label="item.name+'('+item.id+')'" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="所属区域" prop="upid">
        <el-select v-model="temp.upid" placeholder="选择所属区域">
          <el-option v-for="(item,index) in areaOptions" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="工作区域名称" prop="name">
        <el-input placeholder="输入工作区域名称" v-model="temp.name"></el-input>
      </el-form-item>
...
<script>
import { fetchWorkAreaList, fetchWorkArea, createWorkArea, updateWorkArea ,fetchShopAreaList} from '@/api/shop'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchWorkAreaList(this.listQuery).then(response => {
    ...
    getArea(){
     fetchShopAreaList({sid:this.temp.sid}).then(response => {
      this.areaOptions = response.data.list
      this.temp.upid = undefined;
      console.log(this.areaOptions)
    })
    },
    getArea1(){
    //   dict('sArea', 1,0,this.listQuery.sid).then(response => {
    //   this.areaOptions = response
    //   console.log(this.areaOptions)
    // })
    this.getList()
    ...
    handleUpdate(row) {
      fetchWorkArea(row.id).then(response => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          let tempData = Object.assign({}, this.temp)
          tempData = this.remapData(tempData)
          updateWorkArea(tempData.id, tempData).then(() => {
